...
$src = 'f6ca1c7d5d00a2a3fb4ea2f7edfa0f96a6d09c11717f39facabad2d724f16fbb/images/' . $post_data['directory'] . '/' . $post_data['image'];

?>
<div class="media-wrapper" style="margin-right: 70px">
    <?= in_array($ext, ['webm', 'mp4'])
        ? '<video controls src="' . $src . '" id="image"/>'
        : '<img alt="img" src="' . $src . '" id="image" onclick="Note.toggle();"/>';
    ?>
</div>
<?php
...
<form method="post" action="./public/edit_post.php" id="edit_form" name="edit_form" style="display:none">
    <table>
        <tr>
            <td>
                Rating<br/>
                <input type="radio" name="rating" <?php if ($post_data['rating'] == "Explicit") {
                    print 'checked="checked"';
                } ?> value="e"/>Explicit
                <input type="radio" name="rating" <?php if ($post_data['rating'] == "Questionable") {
                    print 'checked="checked"';
                } ?> value="q"/>Questionable
                <input type="radio" name="rating" <?php if ($post_data['rating'] == "Safe") {
                    print 'checked="checked"';
                } ?> value="s"/>Safe
            </td>
        </tr>
        ...
        if ($data !== false && $data != "" && $got_permission === false) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                ?>
                (<?php $row['spam'] == false ? print "<a id=\"rc" . $row['id'] . "\"></a><a href=\"#\" id=\"rcl" . $row['id'] . "\" onclick=\"Javascript:spam('comment','" . $row['id'] . "')\">Report as spam</a>)" : print "<b>Reported</b>)";
                ...
                <script type="text/javascript">
                    //<![CDATA[
                    posts[<?php echo $id;?>].comments[<?php echo $row['id'];?>] = {
                        'score':<?php echo $row['score'];?>,
                        'user': '<?php echo str_replace('\\', "&#92;", str_replace(' ', '%20', str_replace("'", "&#039;", $row['user'])));?>'
                    }
                    //]]>
                </script>
                <?php
